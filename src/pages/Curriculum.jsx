...
import {createNewInterface} from "../functions/newElements.js";
...
export default function Curriculum() {
    ...
    function fetchData() {
        ...
        axios.post("http://localhost:4000/getActivities", {
            courseID: urlParams.get('param1'),
        }).then(function(response){
            return response.data
        }).then((response) => {
            setAssignments(response)
            setAssignmentKeys(Object.keys(response))
        }).catch(function (error) {
            console.log(error)
        })
    }
                ...
                <div className="row" style={{paddingBottom:"18px"}}>
                    ...
                    <div id="rightCol" className="col">
                        ...
                        <div className={"panel"} id={"subInterfaces"} hidden>
                            <div id={"weekPanel0"} style={{textAlign: 'center'}} hidden>
                                <h1 style={{color:"#00006c", textAlign:"center", fontWeight:"bold"}}>All Content</h1>
                                <ul style={{textAlign: 'left'}}>
                                    {assignmentKeys.map((label) => (
                                        <li key={parseInt(label)}><a href={assignments[label]['activityPath']}>{assignments[label]['activityName']}</a></li>
                                    ))}
                                </ul>
                            <button className={'newCourse'} style={{margin:'20px'}}>Add Activity</button>
                            </div>
                        </div>
